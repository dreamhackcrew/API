...
class EpiDatabase
{
  ...
  public function allone($sql = false, $params = array())
  {
    $this->init();
    try
    {
  ...
  public function insert($table, $data, $primary=null, $returnFields=null) {
      $this->init();
      ...
      try {
          ...
          if ( $primary ) {
              ...
              return $this->one("SELECT ".implode(', ',$returnFields)." FROM $table WHERE `$primary`=?", array($key));
          }
  ...
  public function insertOrUpdate($table, $data, $primary) {
      $this->init();

      $select = $fields = array();

      // Make an insert pattern
      foreach(array_keys($data) as $key) {
          $fields[$key] = "`$key`=:$key";
          $select[] = "`$key`";
      }
      ...
      foreach($primary as $key) {
          $primarySearch[] = "`$key`=:$key";
          $primaryValues[$key] = $data[$key];
      }

      // Check if the row exists 
      if ( $this->one("SELECT ".reset($primary)." FROM $table WHERE ".implode(' AND ',$primarySearch),$primaryValues )  ) {
          foreach($primary as $key) {
              unset($fields[$key]);
          }

          $sql = "UPDATE $table SET ".implode(', ',$fields)." WHERE ".implode(' AND ',$primarySearch);
      } else {
          $sql = "INSERT INTO $table SET ".implode(', ',$fields);
      }
  ...
  public function update($table, $data, $where, $params,  $primary=null, $returnFields=null) {
      $this->init();
      ...
      try {
          ...
          if ( $primary ) {
              ...
              return $this->one("SELECT ".implode(', ',$returnFields)." FROM $table $where", $params);
          }
      } catch(PDOException $e) {
		  if ( Epi::getSetting('showSql') )
			  EpiException::raise(new EpiDatabaseQueryException("Query error: {$e->getMessage()} - {$sql}"));
